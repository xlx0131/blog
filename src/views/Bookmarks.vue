  ...
  <div class="min-h-[100dvh] bg-[#fafafa] pt-24 pb-20">
    <div class="max-w-[1400px] mx-auto px-6 sm:px-10 lg:px-20">
      <div class="animate-fade-up">
        ...
        <p class="mt-2 text-base text-neutral-500">{{ bookmarks.length }} 个分类，共 {{ bookmarks.reduce((sum, c) => sum + c.items.length, 0) }} 个网站</p>
      </div>

      <div class="mt-12 space-y-16">
        <section v-for="cat in bookmarks" :key="cat.name" class="animate-fade-up">
          <h2 class="text-lg font-bold tracking-tight text-emerald-700">{{ cat.name }}</h2>
          <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            <div
              v-for="item in cat.items"
              :key="item.id"
              class="block rounded-xl border border-neutral-200 bg-white p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_24px_-8px_rgba(0,0,0,0.08)] tilt-card cursor-pointer"
              @click="openDetail(item)"
            >
              <div class="flex items-center gap-3 mb-2">
                <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-100 to-emerald-50 text-emerald-700 flex items-center justify-center text-sm font-bold truncate">{{ item.name.slice(0, 2) }}</div>
              </div>
              <h4 class="text-sm font-semibold text-neutral-900 truncate">{{ item.name }}</h4>
              <p class="mt-1 text-xs text-neutral-400 truncate line-clamp-2">{{ item.desc }}</p>
            </div>
          </div>
        </section>
      </div>
