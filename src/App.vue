...
<template>
  <div class="min-h-[100dvh] flex flex-col bg-[#0d1117]">
    <!-- Header -->
    <header
      v-if="route.path !== '/'"
      class="fixed top-0 left-0 right-0 z-50 transition-all duration-500"
      :class="scrolled
        ? 'bg-[#161b22]/80 backdrop-blur-xl border-b border-[#30363d]'
        : 'bg-transparent'"
    >
      <div class="max-w-[1400px] mx-auto px-6 lg:px-10 flex items-center justify-between h-16 lg:h-20">
        <!-- Logo -->
        <button
          class="text-lg font-semibold tracking-tight text-[#e6edf3] hover:text-[#8b949e] transition-colors"
          @click="router.push('/')"
        >
          许立鑫
        </button>
        ...
        >
          <span
            class="absolute top-[5px] left-0 w-full h-[1.5px] bg-[#e6edf3] transition-all duration-300"
            :class="mobileOpen ? 'rotate-45 top-[11px]' : ''"
          />
          <span
            class="absolute top-[11px] left-0 w-full h-[1.5px] bg-[#e6edf3] transition-all duration-300"
            :class="mobileOpen ? 'opacity-0' : ''"
          />
          <span
            class="absolute top-[17px] left-0 w-full h-[1.5px] bg-[#e6edf3] transition-all duration-300"
            :class="mobileOpen ? '-rotate-45 top-[11px]' : ''"
          />
        </button>
      </div>
      ...
      <Transition name="mobile">
        <nav v-if="mobileOpen" class="md:hidden border-t border-[#30363d] bg-[#161b22]/95 backdrop-blur-xl">
    ...
    <!-- Footer -->
    <footer v-if="route.path !== '/'" class="border-t border-[#30363d] mt-24">
      <div class="max-w-[1400px] mx-auto px-6 lg:px-10 py-12">
        <div class="flex flex-col md:flex-row items-start justify-between gap-8">
          <div>
            <p class="text-base font-semibold tracking-tight text-[#e6edf3]">许立鑫</p>
            <p class="text-sm text-[#8b949e] mt-1 max-w-xs leading-relaxed">
             写代码、做设计、思考人生。
            </p>
          </div>
          <div class="flex gap-8 text-sm text-[#8b949e]">
            ...
            <RouterLink to="/bookmarks" class="hover:text-[#e6edf3] transition-colors">收藏夹</RouterLink>
          </div>
        </div>
        <div class="border-t border-[#30363d] mt-8 pt-6 text-xs text-[#6e7681]">
